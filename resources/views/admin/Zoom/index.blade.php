...
<h2 class="text-2xl font-bold mb-6">Verifikasi Permintaan Fasilitas Zoom</h2>
...
<div class="overflow-auto">
    <table class="w-full text-sm bg-slate-800 rounded-xl overflow-hidden">
        ...
        <tbody>
            @forelse ($semua as $item)
                <tr class="border-t border-slate-700 hover:bg-slate-700/50">
                    ...
                    <td class="p-3 space-x-2">
                        @if ($item->status === 'menunggu')
                            <form action="{{ route('admin.zoom.verifikasi', $item->id) }}" method="POST" class="inline">
                                @csrf
                        ...
                        @elseif ($item->status === 'disetujui')
                            <form action="{{ route('admin.zoom.selesai', $item->id) }}" method="POST" class="inline">
                                @csrf
            ...
            @empty
                <tr>
                    <td colspan="7" class="p-4 text-center text-slate-400">Tidak ada permintaan Zoom.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
